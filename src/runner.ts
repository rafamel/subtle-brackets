...
export class Runner {
    ...
    private parseUntilComplement(kpObj: IKeepParsingObj, first: boolean = true): number[] {
        let lastChar: number;
        ...
        for (let ii = 0; ii < kpObj.lineText.length; ii++) {
            ...
            if (first && kpObj.direction > 0) {
                ...
                lineTill = lineTill
                    .slice(0, lastChar + (lineTill.length - kpObj.lineText.length));
            } else {
                lineTill = kpObj.lineText.slice(0, lastChar);
            }
            const [isA, isB] = [char === kpObj.aBracket, char === kpObj.bBracket];

            if ((isA || isB) && this.isPunctuationType(kpObj.knownLanguage,
                kpObj.typesFound, kpObj.parsedDocBrackets, kpObj.line, char, lineTill)) {
                if (isA) kpObj.open++;
